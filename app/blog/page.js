...
export default function Blog() {
  const contentDir = path.join(process.cwd(), 'content'); // Get absolute path to content folder
  const files = fs.readdirSync(contentDir);
  const blogs = files.map(file => {
    const filePath = path.join(contentDir, file); // Use path.join for better compatibility
